...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-xs">
      ...
      <div className="bg-stone-950 text-white text-[11px] py-1.5 px-3 sm:px-6 lg:px-8 border-b border-red-950/40">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 font-medium text-stone-300 min-w-0">
            <span className="inline-block w-2 h-2 rounded-full bg-red-500 animate-pulse shrink-0" />
            <span className="tracking-wide truncate hidden sm:inline">Singapore Overseas Recruitment & Placement Support</span>
            <span className="tracking-wide truncate sm:hidden">Singapore Overseas Recruitment</span>
          </div>

          <div className="flex items-center gap-2.5 sm:gap-4 text-xs font-semibold shrink-0">
            {isAdmin && (
              <button
                type="button"
              ...
              >
                ...
                <span className="hidden sm:inline">Admin Logo Control</span>
        ...
        <div className="flex items-center justify-between min-h-16 sm:h-20 py-2 sm:py-0 gap-2">
          ...
          <div className="flex items-center gap-2 min-w-0">
            <button
              ...
              className="flex items-center gap-2 sm:gap-3 text-left group focus:outline-hidden cursor-pointer min-w-0"
            >
              <div
                id="brand-logo-emblem-box"
                className={`w-10 h-10 sm:w-12 sm:h-12 ${settings.logoEmblemShape || 'rounded-xl'} flex items-center justify-center bg-white shadow-xs group-hover:shadow-md group-hover:scale-105 transition-all duration-200 border border-slate-200 shrink-0 relative overflow-hidden`}
              >
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-base sm:text-xl font-black text-slate-900 tracking-tight leading-tight truncate">
                    {settings.logoTitle || 'ARUDHRA'}{' '}
                    <span className="text-red-900 font-bold">{settings.logoSubtitle || 'CONSULTANCY'}</span>
                  </span>
                </div>
                <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium truncate max-w-[130px] xs:max-w-[200px] sm:max-w-xs">
            ...
            {currentTab !== 'home' && (
              <button
                ...
                onClick={() => goBack('home')}
                className="hidden xs:flex items-center gap-1 px-2 sm:px-3 py-1.5 text-xs font-bold text-slate-700 hover:text-red-900 bg-slate-100/90 hover:bg-red-50 border border-slate-200/80 rounded-xl transition-all cursor-pointer group shadow-2xs shrink-0"
              ...
              >
                ...
                <kbd className="hidden md:inline-block px-1 py-0.2 text-[9px] font-mono bg-white text-slate-500 rounded border border-slate-200">
                  Esc
                </kbd>
              </button>
            )}
            ...
            {navLinks.map(link => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 xl:px-3 py-2 rounded-lg text-xs xl:text-sm font-semibold transition-all cursor-pointer whitespace-nowrap ${
                    active
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                {isAdmin ? (
                  <button
                    ...
                    className={`flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                      currentTab === 'admin'
            ...
            <div className="flex items-center lg:hidden">
              <button
                ...
                className="p-1.5 sm:p-2 rounded-lg text-slate-700 hover:text-slate-950 hover:bg-slate-100 focus:outline-hidden cursor-pointer"
