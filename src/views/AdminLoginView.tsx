...
export const AdminLoginView: React.FC = () => {
  ...
  return (
    <div className="min-h-[85vh] bg-stone-900 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className={`w-full mx-auto relative z-10 transition-all duration-300 ${step === '2fa' && showQrCode ? 'max-w-xl' : 'max-w-md'} space-y-4`}>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-stone-400 mt-2">
            {step === 'credentials'
              ...
              : 'Scan the QR code with Google Authenticator or Microsoft Authenticator, then enter your 6-digit TOTP code.'}
          </p>
        </div>
        ...
        <div className="bg-stone-950/90 backdrop-blur-xl rounded-2xl border border-stone-800 shadow-2xl p-6 sm:p-8">
          ...
          {step === 'credentials' ? (
            ...
            <form onSubmit={handle2faSubmit} className="space-y-5">
              ...
              {/* 2FA Authenticator QR Code Plate with Show/Hide toggle */}
              {showQrCode ? (
                <div className="space-y-3 p-4 bg-stone-900/90 border border-emerald-900/50 rounded-2xl shadow-inner">
                  <div className="flex items-center justify-between pb-2 border-b border-stone-800">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-stone-800 hover:bg-stone-700 text-stone-300 hover:text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
                      title="Hide QR code if already scanned"
                    >
                  ...
                  <p className="text-xs text-stone-300">
                    Scan this QR code with <span className="text-emerald-400 font-semibold">Google Authenticator</span>, <span className="text-emerald-400 font-semibold">Microsoft Authenticator</span>, or <span className="text-emerald-400 font-semibold">Apple Passwords</span> on your smartphone.
                  </p>

                  <Admin2faQRCode
                    username={username || 'info@arudhraconsultancy.com'}
                    otpAuthUri={otpAuthUri}
                    secretKey={secretKey}
                  />
                </div>
              ) : (
                <div className="p-4 bg-stone-900/90 border border-stone-800 rounded-xl flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-white">Authenticator App Active</h4>
                      <p className="text-[11px] text-stone-400">Google Authenticator / Microsoft Authenticator</p>
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={() => setShowQrCode(true)}
                    className="px-3 py-1.5 bg-stone-800 hover:bg-stone-700 text-emerald-400 hover:text-emerald-300 border border-stone-700 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shrink-0"
                  >
